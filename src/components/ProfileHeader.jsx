...
const ProfileHeader = ({ userData, onSave, isOwnProfile }) => {
	...
	// Mock data for connection status and connection handling
	const connectionStatus = { status: "connected", requestId: "123" }; // Possible values: connected, pending, received, not_connected
	const isConnected = true; // Mock connected state
	...
	const renderConnectionButton = () => {
		...
		switch (getConnectionStatus) {
			case "connected":
				return (
					<div className='flex gap-2 justify-center'>
						...
						<button className={`${baseClass} bg-red-500 hover:bg-red-600`} onClick={removeConnection}>
			...
			case "received":
				return (
					<div className='flex gap-2 justify-center'>
						<button className={`${baseClass} bg-green-500 hover:bg-green-600`} onClick={acceptRequest}>
							Accept
						</button>
						<button className={`${baseClass} bg-red-500 hover:bg-red-600`} onClick={rejectRequest}>
							Reject
						</button>
					</div>
				);
			default:
				return (
					<button className='bg-primary hover:bg-primary-dark text-white py-2 px-4 rounded-full transition duration-300 flex items-center justify-center' onClick={sendConnectionRequest}>
